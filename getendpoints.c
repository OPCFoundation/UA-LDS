            ...
            for (i=0; i<numEndpoints; i++)
            {
                ...
                for (j=0; j<pEP[i].nNoOfSecurityPolicies; j++)
                {
                    ...
                    for (k=0, mode=1; k<3; k++, mode<<=1)
                    {
                        ...
                        /* fill endpoint url */
						OpcUa_String_AttachReadOnly(&pResponse->Endpoints[index].EndpointUrl, (char*)pEP[i].szUrl);
                        /* fill application description */
                        OpcUa_String_AttachReadOnly(&pResponse->Endpoints[index].Server.ApplicationName.Locale, "en-US");
						OpcUa_String_AttachReadOnly(&pResponse->Endpoints[index].Server.ApplicationName.Text, (char*)ualds_applicationname("en-US"));
						strlcpy(szApplicationUri, (char*)ualds_serveruri(), UALDS_CONF_MAX_URI_LENGTH);
                        ...
                        OpcUa_String_AttachReadOnly(&pResponse->Endpoints[index].Server.ProductUri, (char*)ualds_producturi());
