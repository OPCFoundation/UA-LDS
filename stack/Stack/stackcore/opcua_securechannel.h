...
{ \
    ...
    OpcUa_Trace(OPCUA_TRACE_LEVEL_INFO, "OPCUA_SECURECHANNEL_LOCK:   --> %i!\n", xSecureChannel->iSyncAccessLevel); \
}
...
{ \
    ...
    xSecureChannel->iSyncAccessLevel--; \
    OpcUa_Trace(OPCUA_TRACE_LEVEL_INFO, "OPCUA_SECURECHANNEL_UNLOCK: <-- %i!\n", xSecureChannel->iSyncAccessLevel); \
...
#define OPCUA_SECURECHANNEL_STARTING_SEQUENCE_NUMBER    0
 ...
 *                              [in] SequenceNumber
 *
 * @return result of sequence number check
 */
OpcUa_StatusCode OpcUa_SecureChannel_CheckSequenceNumber(
    struct _OpcUa_SecureChannel*    pSecureChannel,
    OpcUa_UInt32                    uSequenceNumber);

typedef OpcUa_StatusCode (OpcUa_SecureChannel_PfnCheckSequenceNumber)(
    struct _OpcUa_SecureChannel*    pSecureChannel,
    OpcUa_UInt32                    uSequenceNumber);
...
OpcUa_StatusCode OpcUa_SecureChannel_SetPendingInputStream(
    struct _OpcUa_SecureChannel*    pSecureChannel,
    struct _OpcUa_Stream*           pSecureIStream);
...
OpcUa_StatusCode OpcUa_SecureChannel_GetPendingInputStream(
    struct _OpcUa_SecureChannel*    pSecureChannel,
    struct _OpcUa_Stream**          ppSecureIStream);
...
struct _OpcUa_SecureChannel
{
    ...
    /** @brief Check a new sequence number for this channel. */
    OpcUa_SecureChannel_PfnCheckSequenceNumber*     CheckSequenceNumber;
    ...
    struct _OpcUa_Stream*                           pPendingSecureIStream;
    /** @brief The sequence number of the last chunk received. */
    OpcUa_UInt32                                    uLastSequenceNumberRcvd;
    ...
    /** @brief The pending send buffers protected by the Write Mutex. */
    OpcUa_Boolean                                   bAsyncWriteInProgress;
    struct _OpcUa_BufferList*                       pPendingSendBuffers;
    /** @brief Server emulates the blocking behaviour if a response is sent from the read handler. */
    OpcUa_UInt32                                    uPendingMessageCount;
    /** @brief Stores the peer information. */
    OpcUa_String                                    sPeerInfo;
};
